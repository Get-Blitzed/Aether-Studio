...
const OFFLINE_SAFE_KINDS = new Set<ProviderKind>(["mock", "sapi-voice"]);
 ...
 * and the native Windows SAPI voice provider never touch the network, so
 * they're exempt -- this is the one place that distinction is made, rather
 * than duplicating an offline check inside every networked provider
 * implementation.
 */
